...
import keys from "../../gate-453013-17e9861a5457.json";
import { NextApiRequest, NextApiResponse } from "next";
import validator from "email-validator";

interface MailingListRequest extends NextApiRequest {
  body: {
    email: ReadableStream<{ email: string }>;
  };
}

export async function POST(request: MailingListRequest) {
  // For example, fetch data from your DB here

  const auth = new google.auth.GoogleAuth({
    projectId: "gate-453013",
